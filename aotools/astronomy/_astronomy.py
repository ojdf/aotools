...
def photons_per_mag(mag, mask, pxlScale, expTime, waveband='V', bandWidth=None):
        '''
        Calculates the photon flux for a given aperture, star magnitude and wavelength band
        ...
        Parameters:
            ...
            bandWidth(float): bandwidth of wavelength band in nanometres
        ...
        # if bandwidth is not defined use photometric bandwidth from flux_dictionary
        if bandWidth is None:
            bandWidth = flux_dictionary[waveband][1]

        #Area defined m
        area = mask.sum() * pxlScale**2

        # Flux density
        flux_Jy = flux_dictionary[waveband][2] * 10 ** (-0.4 * float(mag))
        flux_photons = flux_Jy * 1.51E7 * bandWidth  # photons sec^-1 m^-2
